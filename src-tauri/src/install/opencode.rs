//! The OpenCode installation adapter (§7.15, INST-01..08, ADPT-04, ADPT-06 item 4).
//!
//! One modification, one file, and the Claude Code adapter's syntax: OpenCode's configuration
//! is JSON, edited through [`super::json`] with its rule that whatever is already in the file
//! is still there afterwards, in the same order (INST-04).
//!
//! | File | Location | What |
//! |---|---|---|
//! | `~/.config/opencode/opencode.json` (under `$XDG_CONFIG_HOME` when it is set; `opencode.json` in a project) | `mcp.handoff` | the local entry with the fixed launcher path, our two environment variables and the per-server `timeout` |
//!
//! Four facts measured against OpenCode 1.18.29 decide that shape (T-074, and
//! `docs/agent-facts.md` of `handoff-mcp`):
//!
//! - **There is no hook to register.** OpenCode offers no command it runs at the end of a turn
//!   and obeys — its extension point is plugins, code inside its own process — so the
//!   capability row says `stop_hook: false` and this adapter makes one modification. The
//!   consent screen has one line.
//! - **No permission is needed.** OpenCode calls an MCP tool without asking, so the entry
//!   carries nothing like Codex's approval mode, and the consent line grants nothing beyond the
//!   entry itself.
//! - **The timeout is in milliseconds.** `"timeout": 1800000` is the thirty minutes of
//!   INST-03, and it matters more here than for the other agents: with nothing configured
//!   OpenCode cuts a call after sixty seconds. `HANDOFF_TOOL_TIMEOUT_MS` mirrors it so the
//!   server heartbeats a minute before it (§5.6).
//! - **The command is one array and the variables are `environment`**, not `command` plus
//!   `args` and `env` as for Claude Code: `{ "type": "local", "command": [<fixed path>], … }`.
//!
//! # Which file
//!
//! OpenCode reads `config.json`, `opencode.json` and `opencode.jsonc` of its configuration
//! folder and merges them, in that order. This adapter writes **`opencode.json`**, and never a
//! `.jsonc`: that is where comments are allowed, and a JSON parse would lose them, which is the
//! one thing INST-04 forbids. A comment in `opencode.json` itself — OpenCode accepts one there
//! too — makes the file one this adapter refuses rather than rewrites
//! ([`InstallError::Malformed`]), the rule every adapter keeps for a file it cannot read.
//! The folder is `$XDG_CONFIG_HOME/opencode`, else `~/.config/opencode` — on Windows too, which
//! is where OpenCode itself looks (measured with `opencode debug paths`).
//!
//! **Project scope** writes `<project>/opencode.json`, which OpenCode finds by walking up from
//! the folder it is started in and merges over the global configuration.
//!
//! # Recognising ours
//!
...
/// The variable OpenCode resolves its configuration folder from, as XDG prescribes.
pub const ENV_XDG_CONFIG_HOME: &str = "XDG_CONFIG_HOME";

/// The catalogue key of the one line of the consent screen.
pub const KEY_MCP_ENTRY: &str = "install.opencode.mcpEntry";

/// The folder OpenCode keeps its configuration in, under the XDG configuration folder.
const OPENCODE_FOLDER: &str = "opencode";
...
#[derive(Debug, Clone)]
pub struct OpenCode {
    /// OpenCode's configuration folder: `$XDG_CONFIG_HOME/opencode`, else `~/.config/opencode`.
    config_dir: PathBuf,
...
impl OpenCode {
    ...
    pub fn detected() -> Result<Self> {
        Ok(Self {
            config_dir: config_dir_from(std::env::var_os(ENV_XDG_CONFIG_HOME), &paths::home_dir()),
    ...
    /// The file holding `mcp`: `opencode.json` in OpenCode's configuration folder, or in the
    /// project.
    #[must_use]
    ...
    /// (SRV-19), and OpenCode takes a command and its arguments as one array.
    #[must_use]
...
/// `$XDG_CONFIG_HOME/opencode`, else `~/.config/opencode`: where OpenCode looks for its own
...
/// native install puts `opencode.exe` there.
...
impl InstallAdapter for OpenCode {
    ...
    fn detect(&self, scope: &Scope) -> Detection {
        ...
        // OpenCode's configuration folder counts as well as the file: OpenCode creates the
        // folder the first time it runs, before anybody has written an `opencode.json`.
        let found = opencode_on_path() || self.config_dir.is_dir() || file.exists();
        Detection {
            agent_id: AGENT_ID,
            found,
            config_files: vec![file],
            // Not read, for the reason `opencode_on_path` gives.
            version: None,
        }
    }
...
mod tests {
    ...
    #[test]
    fn the_entry_is_the_one_opencode_was_measured_with() {
        // T-074: a local server, the fixed path as the whole command, our two variables under
        // `environment`, and thirty minutes in milliseconds — what `opencode debug config`
        // read back from the golden file.
        assert_eq!(
    ...
    fn a_windows_path_survives_the_json_it_is_written_into() {
        // JSON escapes every backslash, and OpenCode reads the array back as the path.
        let adapter = OpenCode::with(
            "C:\\Users\\someone\\.config\\opencode",
    ...
    fn the_file_of_each_scope() {
        ...
        assert_eq!(
            ...
            PathBuf::from("/home/someone/.config/opencode/opencode.json")
        );
        assert_eq!(
            adapter.config_file(&Scope::project("/work/project")),
            PathBuf::from("/work/project/opencode.json")
        );
    }

    #[test]
    fn the_folder_follows_the_variable_opencode_reads() {
        ...
        assert_eq!(
            ...
            PathBuf::from("/elsewhere/config").join("opencode")
        );
    ...
    fn the_names_it_writes_survive_project_scope() {
        // OpenCode strips nothing that was measured (T-074: a TOKEN-named variable arrived),
        // but the rule of every adapter costs nothing and the failure it prevents is invisible.
