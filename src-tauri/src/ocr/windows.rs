...
impl OcrEngine for WindowsOcr {
    fn name(&self) -> &str {
        // The vocabulary of `screenshot.ocr_engine`, beside `vision` and `tesseract`.
        "windows"
    }
...
mod tests {
    ...
    mod paint {
        //! Text drawn with GDI, so that the OCR tests read back something a real recogniser
        //! was trained on.
        //!
        //! A hand-drawn bitmap font would make the assertions a test of our own glyphs
        //! rather than of the engine; a committed PNG would be a blob nobody can regenerate.
        //! This renders with a font every Windows install has, at a size a dashboard would
        //! use, and the pixels never leave the test process.
        ...
        use windows::Win32::Graphics::Gdi::{
            ...
            CLIP_DEFAULT_PRECIS, DEFAULT_CHARSET, DEFAULT_PITCH, DIB_RGB_COLORS, FF_SWISS,
            FW_NORMAL, HGDIOBJ, OUT_TT_PRECIS, TRANSPARENT, WHITENESS,
        };
        ...
        pub fn text(width: u32, height: u32, point_size: i32, lines: &[&str]) -> RgbaImage {
            ...
            // SAFETY: every handle created here is selected out and deleted before the
            // function returns, and `bits` is read only while the bitmap is alive and only
            // for the `width * height * 4` bytes `CreateDIBSection` promised.
            unsafe {
                ...
                for (index, line) in lines.iter().enumerate() {
                    ...
                    let y = 8 + i32::try_from(index).expect("a line number")
