...
| End to end | `scripts\workspace\e2e.ps1` | Ten scenarios with a real Claude Code and a real build of the app; with `-Agent codex`, `-Agent opencode`, `-Agent cursor`, `-Agent copilot` or `-Agent kilo-code`, five of them against a real Codex CLI, OpenCode, Cursor Agent CLI, GitHub Copilot CLI or Kilo CLI, after a preflight in which the agent reads the file the installer writes, and for Cursor and Copilot one more that launches Cursor's editor or VS Code and reads the session it registers; run by hand and not in CI: [e2e.md](e2e.md). |
