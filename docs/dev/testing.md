...
| End to end | `scripts\e2e.ps1` (workspace root) | Ten scenarios with a real Claude Code and a real build of the app, and with `-Agent codex` or `-Agent opencode` five of them against a real Codex CLI or OpenCode, after a preflight in which the agent reads the file the installer writes; run by hand and not in CI: [e2e.md](e2e.md). |
