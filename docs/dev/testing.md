...
| End to end | `scripts\e2e.ps1` (workspace root) | Ten scenarios with a real Claude Code and a real build of the app; with `-Agent codex`, `-Agent opencode` or `-Agent cursor`, five of them against a real Codex CLI, OpenCode or Cursor Agent CLI, after a preflight in which the agent reads the file the installer writes, and for Cursor one more that launches Cursor's editor and reads the session it registers; run by hand and not in CI: [e2e.md](e2e.md). |
