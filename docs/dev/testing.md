...
| End to end | `scripts\e2e.ps1` (workspace root) | Ten scenarios with a real Claude Code and a real build of the app, and with `-Agent codex` five of them against a real Codex CLI, after a preflight in which Codex reads the `config.toml` the installer writes; run by hand and not in CI: [e2e.md](e2e.md). |
