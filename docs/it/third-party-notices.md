...
Baton è software proprietario. È costruito con software scritto da altri, e lo distribuisce,
sotto le licenze dei rispettivi autori. Questa pagina lo elenca.

## Il server MCP: `handoff-mcp`

`handoff-mcp.exe`, installato accanto a Baton, è il server MCP open source del sistema di
...
- questi pacchetti npm, come in `handoff-mcp` 0.2.0, la versione che Baton fissa:
...
`models\ocrs\text-detection.rten` e `models\ocrs\text-recognition.rten` sono distribuiti sotto
licenza **Creative Commons Attribuzione-Condividi allo stesso modo 4.0** (CC BY-SA 4.0). La
licenza riguarda i *pesi* dei modelli; le crate `ocrs` e `rten` che li leggono sono MIT OR
Apache-2.0 e compaiono nell'elenco delle crate qui sotto. L'attribuzione, distribuita accanto
ai file come `models\ocrs\LICENSE`, è questa (in inglese, come nel file):
...
La finestra di Baton è costruita con questi pacchetti npm:
...
È disegnata da **Microsoft Edge WebView2**, che fa parte di Windows e non viene distribuito con
Baton: è software di Microsoft, alle condizioni di Microsoft. Quando manca, il programma di
installazione scarica l'installer di Microsoft. Il programma di installazione stesso è
costruito con **NSIS**, sotto licenza zlib/libpng.

## Le crate Rust

Il nucleo di Baton è scritto in Rust. Queste sono le crate con cui viene compilata la versione
per Windows, generate dal grafo delle dipendenze di Cargo per la versione distribuita da
`scripts/third-party-notices.mjs`: non sono elencate le crate usate solo per costruire o per
provare Baton, né quelle dietro funzionalità che la versione distribuita non attiva. Le licenze
sono espressioni SPDX ([spdx.org/licenses](https://spdx.org/licenses/)); il testo della licenza
di ogni crate, con la riga del copyright, è nel pacchetto sorgente della crate su
