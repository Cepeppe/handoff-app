...
and a hook that runs when the agent finishes a turn. It never writes them without showing you
first. This page explains every line.
...
- Everything else in the file stays: every key and value you had, in the same order and with
  the same indentation. Blank lines between entries are not kept.
- Claude Code reads its settings when a session starts, so restart the sessions that were
  already running.
...
content, and only sessions working in that folder see it.
...
is removed too. The backup copies stay where they are.
